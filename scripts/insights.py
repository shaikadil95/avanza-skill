...
def main():
    ...
    # Normalise: report may be dict or object
    if isinstance(report, dict):
        td = report.get("totalDevelopment")
        period_from = report.get("from") or report.get("from_")
        period_to = report.get("to")
        dr = report.get("developmentResponse")
    else:
        ...
        dr = getattr(report, "developmentResponse", None)
    ...
    if td:
        ...
        print(f"  Total gain  : {fmt(change)} SEK  ({pct_str})")

    # --- Breakdown: development vs dividends ---
    if dr:
        if isinstance(dr, dict):
            total_outcome = dr.get("totalOutcome")
        else:
            total_outcome = getattr(dr, "totalOutcome", None)

        if total_outcome:
            if isinstance(total_outcome, dict):
                dev = total_outcome.get("development")
                div = total_outcome.get("dividends")
                total = total_outcome.get("total")
            else:
                dev = getattr(total_outcome, "development", None)
                div = getattr(total_outcome, "dividends", None)
                total = getattr(total_outcome, "total", None)
    ...
    if dr:
        if isinstance(dr, dict):
            bw = dr.get("bestAndWorst")
        else:
            bw = getattr(dr, "bestAndWorst", None)

        if bw:
            if isinstance(bw, dict):
                best = bw.get("bestPositions") or []
                worst = bw.get("worstPositions") or []
            else:
            ...
            if best:
                ...
                for pos in best[:5]:
                    if isinstance(pos, dict):
                        name = (pos.get("name") or "—")[:col_w[0]]
                        dev_val = pos.get("development")
                        pct = pos.get("totalDevelopmentInPercent")
                        ev = pos.get("endValue")
                    else:
